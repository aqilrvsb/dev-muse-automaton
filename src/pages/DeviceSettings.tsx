...
export default function DeviceSettings() {
  ...
  const [formData, setFormData] = useState({
    ...
    instance: '',
    webhook_id: '',
    provider: 'whacenter' as 'whacenter',
  ...
  const handleToggleWebhook = async (device: Device) => {
    try {
      const newValue = !device.ecom_webhook_enabled

      const { error } = await supabase
        .from('device_setting')
        .update({
          ecom_webhook_enabled: newValue,
          updated_at: new Date().toISOString(),
        })
        .eq('id', device.id)
      ...
      setDevices(devices.map(d =>
        d.id === device.id ? { ...d, ecom_webhook_enabled: newValue } : d
      ))

      await Swal.fire({
        icon: 'success',
        title: newValue ? 'Webhook Enabled!' : 'Webhook Disabled',
        text: newValue ? 'E-Commerce webhook is now active for this device.' : 'E-Commerce webhook has been disabled.',
        timer: 2000,
        showConfirmButton: false,
      })
    } catch (error: any) {
      ...
      await Swal.fire({
        ...
        text: error.message || 'Failed to toggle webhook',
      })
    }
  }
  ...
  const handleSaveWebhookSettings = async () => {
    if (!webhookSettingsDevice) return

    try {
      const { error } = await supabase
        .from('device_setting')
        .update({
          ecom_webhook_stage: webhookFormData.ecom_webhook_stage,
          ecom_webhook_template: webhookFormData.ecom_webhook_template,
          updated_at: new Date().toISOString(),
        })
        .eq('id', webhookSettingsDevice.id)

      if (error) throw error

      // Update local state
      setDevices(devices.map(d =>
        d.id === webhookSettingsDevice.id ? {
          ...d,
          ecom_webhook_stage: webhookFormData.ecom_webhook_stage,
          ecom_webhook_template: webhookFormData.ecom_webhook_template,
        } : d
      ...
      setShowWebhookSettingsModal(false)
      setWebhookSettingsDevice(null)

      await Swal.fire({
        icon: 'success',
        title: 'Settings Saved!',
        text: 'Webhook settings have been updated.',
        timer: 2000,
        showConfirmButton: false,
      })
    } catch (error: any) {
      ...
      await Swal.fire({
        ...
        text: error.message || 'Failed to save webhook settings',
      })
    }
  }
  ...
  return (
    <Layout>
      <div className="p-8">
              ...
              <div key={device.id} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="border-t border-gray-200 pt-4 mb-4">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium text-gray-700">E-Commerce Webhook</span>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={device.ecom_webhook_enabled || false}
                        onChange={() => handleToggleWebhook(device)}
                      ...
                      <div className="bg-gray-50 border border-gray-200 rounded-lg p-2">
                        ...
                        <div className="flex items-center gap-2">
                          <code className="text-xs text-gray-800 break-all flex-1">
                            https://pening-bot-website.deno.dev/{device.instance}
                          </code>
                          <button
                            onClick={() => {
                              navigator.clipboard.writeText(`https://pening-bot-website.deno.dev/${device.instance}`)
                              Swal.fire({
                                icon: 'success',
                                title: 'Copied!',
        ...
        {showWebhookSettingsModal && webhookSettingsDevice && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-xl">
              <div className="p-6">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-2xl font-bold text-gray-900">E-Commerce Webhook Settings</h3>
                  <button
                    onClick={() => {
                      setShowWebhookSettingsModal(false)
                      setWebhookSettingsDevice(null)
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    <p className="text-sm text-gray-600">Device: <span className="font-semibold text-gray-900">{webhookSettingsDevice.device_id}</span></p>
                    <p className="text-xs text-gray-500 mt-1">Instance: {webhookSettingsDevice.instance}</p>
                  </div>

                  {/* Webhook URL */}
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Webhook URL (for WooCommerce)</label>
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        value={`https://pening-bot-website.deno.dev/${webhookSettingsDevice.instance}`}
                        readOnly
                        className="flex-1 bg-gray-100 border border-gray-300 text-gray-800 rounded-lg px-4 py-2 text-sm"
                      />
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(`https://pening-bot-website.deno.dev/${webhookSettingsDevice.instance}`)
                          Swal.fire({
                            icon: 'success',
                            title: 'Copied!',
                            text: 'Webhook URL copied to clipboard',
                    ...
                    <p className="text-xs text-gray-500 mt-1">Use this URL in WooCommerce Webhooks with topic "order.updated"</p>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      value={webhookFormData.ecom_webhook_template}
                      onChange={(e) => setWebhookFormData({ ...webhookFormData, ecom_webhook_template: e.target.value })}
                      className="w-full bg-white border border-gray-300 text-gray-900 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-500 font-mono text-sm"
                      rows={8}
                    ...
                    <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <code className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">{'{{name}}'}</code>
                        <code className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">{'{{phone}}'}</code>
                        <code className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">{'{{order_id}}'}</code>
                        <code className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">{'{{product}}'}</code>
                        <code className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">{'{{total}}'}</code>
                        <code className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">{'{{status}}'}</code>
                        <code className="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">{'{{address}}'}</code>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <h4 className="font-bold text-yellow-900 mb-2">WooCommerce Setup Instructions:</h4>
                    <ol className="list-decimal list-inside space-y-1 text-sm text-yellow-800">
                      <li>Go to WooCommerce → Settings → Advanced → Webhooks</li>
                      <li>Click "Add webhook"</li>
                      <li>Set Name (e.g., "PeningBot Order")</li>
                      <li>Set Status to "Active"</li>
                      <li>Set Topic to "Order updated"</li>
                      <li>Paste the Webhook URL above in "Delivery URL"</li>
                      <li>Set Secret to your device instance ID for security</li>
