...
// Load conversations from wasapbot table
        ...
        if (data.success && data.conversations && data.conversations.length > 0) {
            ...
            conversationsList.innerHTML = `
                <div class="empty-state">
                    ...
                    <p class="empty-state-text">Start a conversation with your WhatsApp Bot to see it here</p>
                </div>
            `;
        }
    } catch (error) {
            ...
            <table class="devices-table">
                ...
                <tbody>
                    ${conversations.map((conv, index) => {
                        ...
                        // Reply Status: based on execution_status
                        const replyStatus = conv.execution_status || 'active';
                        const replyBadgeClass = conv.execution_status === 'completed' ? 'badge-human' : 'badge-ai';
...
function calculateAnalytics(conversations) {
    ...
    // Active flows (execution_status is 'active' or not 'completed')
    const activeFlows = conversations.filter(c => c.execution_status !== 'completed').length;
    const activePercent = ((activeFlows / total) * 100).toFixed(1);

    document.getElementById('activeFlows').textContent = activeFlows;
    document.getElementById('flowsChange').textContent = `${activePercent}% of total`;

    // Waiting for reply
    const waitingReply = conversations.filter(c => c.waiting_for_reply === true || c.waiting_for_reply === 1).length;
    const waitingPercent = ((waitingReply / total) * 100).toFixed(1);

    document.getElementById('waitingReply').textContent = waitingReply;
    document.getElementById('waitingChange').textContent = `${waitingPercent}% of total`;
