...
// ============================================================================
// FACEBOOK ID DETECTION - Detect if number is Facebook ID vs real phone
// ============================================================================

/**
 * Check if a phone number is actually a Facebook/Meta ID
 * Facebook IDs are typically:
 * - More than 14 digits
 * - Don't start with 6 or 0 (Malaysian phone numbers start with 6 or 0)
 */
function isFacebookId(phone: string): boolean {
  if (!phone) return false;

  // Remove any non-digit characters
  const cleanPhone = phone.replace(/\D/g, '');

  // Check if more than 14 digits
  if (cleanPhone.length > 14) {
    return true;
  }

  // Check if doesn't start with 6 or 0 (Malaysian phone format)
...
async function executePromptBasedFlow(params: {
  ...
  // Step 4.1: Check if stored phone is Facebook ID and replace with real phone
  // Facebook IDs are long numbers (>14 digits) or don't start with 6/0
  if (conversation && isFacebookId(conversation.prospect_num)) {
    console.log(`⚠️  Stored phone is Facebook ID: ${conversation.prospect_num}`);

    // Check if incoming 'phone' parameter is a real phone (not Facebook ID)
    if (!isFacebookId(phone)) {
      console.log(`📱 Incoming phone is real: ${phone}`);

      // Update prospect_num with real phone
      const { error: updatePhoneError } = await supabaseAdmin
        .from("ai_whatsapp")
        .update({ prospect_num: phone })
        .eq("id_prospect", conversation.id_prospect);

      if (!updatePhoneError) {
        console.log(`✅ Replaced Facebook ID with real phone: ${phone}`);
        // Update local conversation object too
        conversation.prospect_num = phone;
      } else {
        console.error(`❌ Failed to update phone:`, updatePhoneError);
      }
    } else {
      console.log(`ℹ️  Incoming phone is also Facebook ID, keeping stored value`);
    }
  }
